...
const members = [
  {
    ...
    role: 'PM',
    blurb:
      'Drives the product roadmap and aligns the team around priorities that matter most.',
  ...
  },
  {
    ...
    role: 'DEV',
    blurb:
      'Builds robust, scalable solutions and keeps the codebase clean and maintainable.',
    ...
    <main className="min-h-screen bg-white bg-[radial-gradient(circle_at_1px_1px,rgba(59,130,246,0.22)_1px,transparent_0)] [background-size:18px_18px]">
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-6 py-8">
        ...
        <section className="flex flex-1 flex-col items-center justify-center py-12 text-center">
          <span className="rounded-full bg-blue-50 px-4 py-1 text-xs font-semibold uppercase tracking-wide text-blue-500">
            Sprint Team
          </span>
          ...
          <p className="mt-4 max-w-xl text-sm leading-6 text-slate-500 md:text-base">
            Four specialists. One shared goal. Built to move fast, ship with
            quality, and make every sprint count.
          </p>
          ...
          <div className="mt-10 grid w-full gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {members.map((member) => (
              ...
              >
                <div
                  className={`flex h-16 w-16 items-center justify-center rounded-2xl border text-lg font-bold ${member.avatarClass}`}
                >
                  {member.initials}
                </div>
